...
function Top100Page() {
  ...
  const { playlists } = useSelector((state) => state.playlistTop100);
  const isLoading = useSelector((state) => state.isLoadingTab.isLoading);

  useEffect(() => {
    dispatch(setIsLoadingTab(true));
    getApiTop100Page().then((res) => {
      dispatch(getPlaylistTop100([...res.data.data]));
  ...
  return (
    <div className="app__container tab--top100" ref={containerRef}>
